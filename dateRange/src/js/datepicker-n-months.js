 ...
 */
{
	...
	addValue : function(continuous) {
		...
		if(me.adhereEl) {
			var first = me.getValue().indexOf(',');;
			var last = me.getValue().lastIndexOf(',');
			var datestart = me.getValue().substring(0, parseInt(first));
			var dateend = me.getValue().substring(parseInt(last)+1);
			me.adhereEl.value = datestart + '~' + dateend;
		}
		me.update();
		me.newEv({
			ev : 'change'
